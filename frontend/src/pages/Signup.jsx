...
const Signup = () => {
  ...
  const inputClass = "w-full pl-12 pr-4 py-3.5 bg-gray-50 dark:bg-white/5 border border-transparent dark:border-white/10 focus:bg-white dark:focus:bg-white/10 focus:border-brand/30 focus:ring-4 focus:ring-brand/5 rounded-2xl transition-all outline-none text-gray-700 dark:text-white font-medium";
  const labelClass = "text-sm font-bold text-gray-700 dark:text-gray-300 ml-1 mb-1 block";
  const iconClass = "absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 group-focus-within:text-brand transition-colors";

  return (
    <div className="min-screen flex items-center justify-center px-4 py-20 bg-transparent relative overflow-hidden">
      {/* Decorative Elements */}
      ...
      <div className="max-w-2xl w-full relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium">Join our community of premium tech enthusiasts</p>
        </div>

        <div className="bg-white dark:bg-gray-900/95 dark:backdrop-blur-2xl p-8 md:p-12 rounded-[3.5rem] border border-gray-100 dark:border-white/5 shadow-2xl">
          {/* Notifications */}
          {error && (
            <div className="mb-8 p-4 bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/50 text-red-600 dark:text-red-400 rounded-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2 text-sm font-bold">
              <AlertCircle size={20} />
              {error}
            </div>
          )}
          {successMessage && (
            <div className="mb-8 p-4 bg-green-50 dark:bg-green-950/30 border border-green-100 dark:border-green-900/50 text-green-600 dark:text-green-400 rounded-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2 text-sm font-bold">
